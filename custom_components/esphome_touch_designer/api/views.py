...
def compile_to_esphome_yaml(device: DeviceProject) -> str:
    ...
    # Prepend assets (images/fonts) if any
    if assets_yaml.strip():
        recipe_text = assets_yaml.rstrip() + "\n\n" + recipe_text

    # Inject HA bindings at marker or prepend
    if "#__HA_BINDINGS__" in recipe_text:
        ...
        recipe_text = ha_bindings_yaml.rstrip() + "\n\n" + recipe_text

    merged = _inject_pages_into_recipe(recipe_text, pages_yaml)

    header = (
        f"# Generated by {DOMAIN} v0.48.0\n"
...
class DevicesView(HomeAssistantView):
    ...
    async def post(self, request):
        ...
        device = DeviceProject(
            ...
            project=body.get("project") or DeviceProject.__dataclass_fields__["project"].default_factory(),  # type: ignore
        )
...
def _extract_recipe_metadata_from_text(recipe_text: str) -> dict:
    ...
    try:
        ...
        if isinstance(model, dict):
            return _extract_recipe_metadata(model, recipe_text, label=None)
    except Exception:
        pass
    # Fallback: just try to detect resolution
...
class DeviceExportPreviewView(HomeAssistantView):
    ...
    async def post(self, request, device_id: str):
        ...
        device = await storage_get_device(hass, entry_id, device_id)
        if not device:
            return self.json({"ok": False, "error": "device_not_found"}, status_code=404)

        proj = await storage_get_project(hass, entry_id, device_id)
        yaml_text = compile_project_to_esphome_yaml(hass, device, proj or {})
